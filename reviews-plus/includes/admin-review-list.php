...
function ic_force_ic_revs_labels( $translated, $text, $textdomain ) {
    if ( is_admin() && $textdomain == 'default' && ! empty( $text ) ) {
        if ( strpos( $text, 'comments' ) !== false ) {
            $translated = str_replace( 'comments', __( 'reviews', 'reviews-plus' ), $text );
        } else if ( strpos( $text, 'Comments' ) !== false ) {
            $translated = str_replace( 'Comments', __( 'Reviews', 'reviews-plus' ), $text );
...
/**
 * Handles administrative initialization for managing review comments in bulk actions.
 * This function checks the current action, applies appropriate permissions, and
 * performs operations such as approving, unapproving, spamming, unspamming,
 * trashing, untrashing, or deleting comments. It also redirects accordingly
 * based on the actions performed.
 *
 * @return void
 * @global wpdb $wpdb WordPress database access abstraction object.
 *
 * @global IC_Reviews_List_Table $ic_ic_revs_table The instance of the custom reviews table handler.
 */
function ic_reviews_admin_init() {
    ...
    if ( $doaction ) {
        ...
        if ( method_exists( $ic_ic_revs_table, 'get_pagenum' ) ) {
            $redirect_to = add_query_arg( 'paged', $ic_ic_revs_table->get_pagenum(), $redirect_to );
        }
...
//require_once( ABSPATH . 'wp-admin/edit-comments.php' );
    ...
    <div class="wrap">
        ...
        <form id="comments-form" method="get">
            ...
            <input type="hidden" name="pagegen_timestamp"
                   value="<?php echo esc_attr( current_time( 'mysql', 1 ) ); ?>"/>
            ...
            <input type="hidden" name="_total"
                   value="<?php echo esc_attr( $ic_ic_revs_table->get_pagination_arg( 'total_items' ) ); ?>"/>
            <input type="hidden" name="_per_page"
                   value="<?php echo esc_attr( $ic_ic_revs_table->get_pagination_arg( 'per_page' ) ); ?>"/>
            <input type="hidden" name="_page"
                   value="<?php echo esc_attr( $ic_ic_revs_table->get_pagination_arg( 'page' ) ); ?>"/>
